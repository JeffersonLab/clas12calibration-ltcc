...
//                    System.out.println(iSect + "_" + iLay + "_" + iComp);
                    ...
                    H1F htime_wide = new H1F("htime_wide_" + iSect + "_" + iLay + "_" + iComp, 300, -30.0, 30.0);
                    htime_wide.setTitleX("Time (ns)");
                    htime_wide.setTitleY("Counts");
                    htime_wide.setTitle("Channel (" + iSect + "," + iLay + "," + iComp + ")");
                    H1F htime = new H1F("htime_" + iSect + "_" + iLay + "_" + iComp, 300, -3.0, 3.0);
    ...
    public void processEvent(DataEvent event) {
	    ...
	    for(int loop = 0; loop < rows; loop++){
                ...
                int layer   = 0;
                int segment = bank.getShort("segment", loop);
                int nhit    = bank.getShort("nHits", loop);
                ...
                if(phi>-30) {
                    if(phi-60*(sector-1)>0) layer=2;
                    else                    layer=1;
                }
                else {
                    if(-phi-60*(7-sector)>0) layer=1;
                    else                     layer=2;                    
                }
                if(recBankEB!=null && recEvenEB!=null) {
                    ...
                    for(int part = 0; part < nrows; part++){
                        ...
                        if(recParticle.pid()==11) {
                            ...
                            if(Math.abs(phi-Math.toDegrees(recParticle.phi()))<60 && layer>0 && nhit==1 && nFill==0) {
                                nFill++;
                                double timec = time-path/PhysicsConstants.speedOfLight()-startTime;
                                this.getDataGroup().getItem(sector, layer, segment).getH1F("htsum").fill(timec);
                                this.getDataGroup().getItem(sector, layer, segment).getH1F("htime_wide_" + sector + "_" + layer + "_" + segment).fill(timec);
                                this.getDataGroup().getItem(sector, layer, segment).getH1F("htime_" + sector + "_" + layer + "_" + segment).fill(timec);
                                if (this.getPreviousCalibrationTable().hasEntry(sector, layer, segment)) {
                                    ...
                                    this.getDataGroup().getItem(sector, layer, segment).getH1F("htsum_calib").fill(timec - offset);
                                    this.getDataGroup().getItem(sector, layer, segment).getH1F("htime_calib_" + sector + "_" + layer + "_" + segment).fill(timec - offset);
                                }
                            }
                        }
                    }
